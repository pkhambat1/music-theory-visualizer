...
- **Audio:** Tone.js (Salamander piano samples, eager singleton sampler, arpeggiate support)
...
- Extension popover triggered by plus icon (lucide `Plus`) via `Popover` component, with `MultiSelect` for extensions and bass note picker
...
- **Hover lines (all types):** `#000000` stroke, strokeWidth 1.5
- **Static connection lines:** `colors.border` (tinted Set3 gray), strokeWidth 1.5
- **IntervalLabel pills:** white fill (`#ffffff`), black text
